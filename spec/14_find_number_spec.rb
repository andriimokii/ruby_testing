...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:random_number) { double('random_number', value: 5) }
      subject(:game_over) { described_class.new(0, 9, random_number, 5) }
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number) { double('random_number', value: 6) }
      subject(:game_not_over) { described_class.new(0, 9, random_number, 5) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:game) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        expect { game.update_range }.to change(game, :min).from(5).to(8)
      end

      it 'does not update max' do
        expect { game.update_range }.not_to change(game, :max)
      end
    end
  end
end
